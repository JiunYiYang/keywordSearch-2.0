...
for i in range(0, len(KEYWORDS_full)):
    try:
        r = requests.get('http://moz.com/explorer/api/2.5/keyword/suggestions/' + str(KEYWORDS_full[i]) + '?locale=zh-TW&strategy=default', headers=headers, proxies=proxies, auth=proxy_auth)
